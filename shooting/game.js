...
function damageBot(bot,amount,zone,attacker,weapon){
  ...
  if(bot.hp<=0){if(bot.rangeTarget){const now=performance.now()/1000;bot.lastTakedownSeconds=Math.max(.01,now-bot.takedownStartedAt);bot.takedownStartedAt=null}beginBotDeath(bot,bot.rangeTarget?1.5:5);bot.parts.forEach(p=>{p.userData._solid=false});if(bot.rangeTarget){game.kills++;addFeed(attacker,bot.name,weapon);return}if(bot.team==='red')game.blue++;else game.red++;if(attacker===game.player.name)game.kills++;addFeed(attacker,bot.name,weapon)}
}
