...
import {buildWeaponModel,buildThrowableModel,addFirstPersonHands,buildOperator,poseOperator,prewarmOperatorTemplates,WEAPON_SPECS} from './visuals.js?v=operator-motion-25';
...
document.addEventListener('mousemove',e=>{if(document.pointerLockElement!==renderer.domElement||game.phase!=='playing')return;const aimScale=game.aiming?Math.max(.22,aimFov()/72):1;game.yaw-=e.movementX*.0019*aimScale;game.pitch=clamp(game.pitch-e.movementY*.0018*aimScale,-1.48,1.48);if(game.view==='third'&&game.playerMoving)game.moveInput='';camera.rotation.set(game.pitch,game.yaw,0,'YXZ')});
...
function updatePlayer(dt){
  if(!game.player.alive){if(game.playerModel)game.playerModel.root.visible=false;return}let f=(game.keys.KeyW?1:0)-(game.keys.KeyS?1:0),r=(game.keys.KeyD?1:0)-(game.keys.KeyA?1:0);const hasInput=!!(f||r),inputSignature=`${f},${r}`,len=Math.hypot(f,r)||1;f/=len;r/=len;
  ...
  let sx,sz;if(game.view==='third'&&hasInput){if(game.moveInput!==inputSignature){const dx=-Math.sin(game.yaw)*f+Math.cos(game.yaw)*r,dz=-Math.cos(game.yaw)*f-Math.sin(game.yaw)*r;game.moveYaw=Math.atan2(-dx,-dz);game.moveInput=inputSignature}const turn=Math.atan2(Math.sin(game.moveYaw-game.yaw),Math.cos(game.moveYaw-game.yaw));game.yaw+=turn*Math.min(1,dt*11);sx=-Math.sin(game.moveYaw)*speed*dt;sz=-Math.cos(game.moveYaw)*speed*dt}else{if(!hasInput)game.moveInput='';sx=(-Math.sin(game.yaw)*f+Math.cos(game.yaw)*r)*speed*dt;sz=(-Math.cos(game.yaw)*f-Math.sin(game.yaw)*r)*speed*dt}
  ...
  if(game.playerModel){const avatar=game.playerModel.root;avatar.visible=game.view==='third';avatar.position.set(p.x,feet,p.z);avatar.rotation.y=game.view==='third'&&moving?game.moveYaw:game.yaw;poseOperator(avatar,{stance:game.stance,moving,gait:moveGait,time:performance.now()/1000,dt})}
...
function updateBots(dt){
  ...
  for(const b of game.entities){
    ...
    const movedDistance=Math.hypot(b.root.position.x-oldX,b.root.position.z-oldZ),moving=movedDistance>.0001;if(wantedSpeed>.8&&(!moved||movedDistance<dt*.18))b.stuckTime+=dt;else b.stuckTime=Math.max(0,b.stuckTime-dt*2.4);if(b.stuckTime>.48){b.avoidSide*=-1;b.escapeTimer=.75+Math.random()*.45;b.navPath=null;b.navRepath=0;b.stuckTime=0;game.aiStats.unstucks++}poseOperator(b.root,{stance:'stand',moving,gait:'walk',time:animT,dt});setBotAnimation(b,moving?'walk':'idle');updateEntityFootstep(b,moving,'walk');
...
window.__RIFT_POSE_DEBUG__=()=>{const avatar=game.playerModel,rig=avatar?.root.userData.operatorRig||[],legs=rig.filter(item=>item.kind==='leg'),delta=legs.reduce((value,item)=>Math.max(value,Math.abs(item.mesh.rotation.x-item.rotation.x)),0);return{stance:game.stance,view:game.view,cameraYaw:game.yaw,facingYaw:avatar?.root.rotation.y||0,modelPitch:avatar?.model.rotation.x||0,modelY:avatar?.model.position.y||0,legRotationDelta:delta,gunInHands:!!avatar?.gun&&avatar.gun.parent===avatar.model,gunPosition:avatar?.gun?avatar.gun.position.toArray():null,scaleY:avatar?.model.scale.y||0}};
