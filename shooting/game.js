...
import {buildWeaponModel,buildThrowableModel,addFirstPersonHands,buildOperator,poseOperator,prewarmOperatorTemplates,WEAPON_SPECS} from './visuals.js?v=held-reload-27';
import {MAPS,RANGE_MAP,buildArena,scaleMap} from './arena.js?v=collision-25';
...
const WEAPONS={
  ...
  awm:{name:'AWM',type:'狙击步枪',damage:184,rate:42,recoil:92,spread:98,mag:5,reserve:25,reload:3.9,range:100,auto:false,bolt:true,color:0x28342d,desc:'威力极高的栓动狙击枪。单发射击，必须拔栓后才能再次开火。'},
  svd:{name:'SVD',type:'狙击步枪',damage:116,rate:120,recoil:75,spread:91,mag:10,reserve:40,reload:3.1,range:92,auto:false,color:0x4a392a,desc:'半自动精确射手步枪，拥有更快的补射能力。'},
  m24:{name:'M24 SWS',type:'狙击步枪',damage:168,rate:48,recoil:84,spread:96,mag:5,reserve:30,reload:3.6,range:98,auto:false,bolt:true,color:0x465344,desc:'稳定可靠的栓动狙击系统，威力与拔栓速度较为均衡。'},
  m82:{name:'M82A1',type:'狙击步枪',damage:220,rate:55,recoil:100,spread:94,mag:10,reserve:20,reload:4.8,range:100,auto:false,color:0x353932,desc:'反器材重型狙击枪，威力冠绝全场但极其笨重、换弹漫长。'},
  kar98:{name:'KAR98K',type:'狙击步枪',damage:176,rate:45,recoil:88,spread:97,mag:5,reserve:25,reload:3.8,range:96,auto:false,bolt:true,color:0x5d4531,desc:'经典栓动步枪，弹道精准且单发伤害极高。'},
  vss:{name:'VSS VINTOREZ',type:'狙击步枪',damage:70,rate:400,recoil:42,spread:88,mag:20,reserve:80,reload:2.8,range:72,auto:false,color:0x493b2f,desc:'亚音速消音精确步枪，伤害较低但补射快速且隐蔽。'},
...
function makeMenuScene(){
  ...
  const {root:hero,model:heroModel}=buildOperator('blue'),gun=buildWorldGun(WEAPONS[save.slots[0]]?.color,save.slots[0],1,weaponAttachments(save.slots[0],0));gun.rotation.y=Math.PI/2;gun.position.set(0,1.21,-.28);heroModel.add(gun);hero.scale.setScalar(1.55);
...
function handleNetwork(msg){
  ...
  if(msg.type==='state'&&net.online){
    ...
    if(bot&&(+msg.seq||0)>=(bot.netSeq??-1)){
      ...
      bot.netYaw=msg.yaw;bot.netStance=msg.stance||'stand';bot.netGait=['run','crouch','prone'].includes(msg.gait)?msg.gait:'walk';bot.netMoving=msg.moving!==false;bot.netReloading=!!msg.reloading;bot.netDeaths=Math.max(bot.netDeaths||0,+msg.deaths||0);bot.hp=msg.hp;bot.armor=msg.armor??bot.armor;setRemoteWeapon(bot,msg.weapon,msg.attachments);
...
function updateRangeUI(){
  ...
  $('#range-status').innerHTML=`无限弹夹：<b>${game.rangeInfiniteMagazine?'开启':'关闭'}</b> · 靠近工作台按 <b>E</b> 设置 · <b>R</b> 切换视角`;
...
function buildBattlefield(map,seed=null){
  ...
  const addCollider=(x,z,w,h,d,kind='prop',baseY=0)=>game.colliders.push({x,z,w,d,base:baseY,top:baseY+h,kind,mesh:null});
  game.mapSeed=seed;buildArena(map,{scene,worldGroup,renderer,addSolid,addCollider,collides,random:seed==null?Math.random:seededRandom(seed)});
  buildViewModel();
}

function setupPlayer(){
  const slots=save.slots,me=net.online?net.room?.players.find(p=>p.id===net.id):null,team=me?.team||'blue',startZ=game.map.range?game.map.size-5:playerSpawnZ(team);game.playerStartZ=startZ;game.feet=0;game.velY=0;game.player={pos:new THREE.Vector3(game.map.range?1.8:0,1.68,startZ),hp:100,armor:50,alive:true,life:1,team,name:me?.name||activeAccount||'你'};game.lastSafe={x:game.player.pos.x,z:game.player.pos.z,feet:0};camera.position.copy(game.player.pos);game.yaw=team==='blue'?0:Math.PI;game.moveYaw=game.yaw;game.pitch=0;camera.rotation.order='YXZ';
...
function rebuildPlayerWorldGun(){const avatar=game.playerModel;if(!avatar)return;if(avatar.gun)avatar.gun.parent?.remove(avatar.gun);avatar.gun=null;if(game.selected<1||game.selected>2)return;const key=game.ammo[game.selected-1]?.key;if(!key)return;const gun=buildWorldGun(WEAPONS[key].color,key,0,weaponAttachments(key,game.selected-1));gun.rotation.y=Math.PI/2;gun.position.set(0,1.21,-.28);avatar.model.add(gun);avatar.gun=gun}
function poseHeldGun(holder,reload=0,dt=1/60){const gun=holder?.gun||holder?.worldGun;if(!gun)return;const smooth=1-Math.exp(-Math.max(.001,dt)*18),x=0,y=1.21-reload*.1,z=-.28+reload*.055,rx=reload*.18,ry=Math.PI/2,rz=reload*.12;gun.position.x+=(x-gun.position.x)*smooth;gun.position.y+=(y-gun.position.y)*smooth;gun.position.z+=(z-gun.position.z)*smooth;gun.rotation.x+=Math.atan2(Math.sin(rx-gun.rotation.x),Math.cos(rx-gun.rotation.x))*smooth;gun.rotation.y+=Math.atan2(Math.sin(ry-gun.rotation.y),Math.cos(ry-gun.rotation.y))*smooth;gun.rotation.z+=Math.atan2(Math.sin(rz-gun.rotation.z),Math.cos(rz-gun.rotation.z))*smooth}
...
function setupPlayerAvatar(){const {root,model}=buildOperator(game.player.team,{castShadow:false});root.position.set(game.player.pos.x,game.feet,game.player.pos.z);worldGroup.add(root);game.playerModel={root,model,gun:null};rebuildPlayerWorldGun()}
function applyViewMode(mode,announce=false){game.view=mode==='third'?'third':'first';preferredView=game.view;if(game.playerModel)game.playerModel.root.visible=game.view==='third'&&game.player.alive;if(viewGun)viewGun.visible=game.view==='first'&&game.selected>=1&&game.selected<=2;const label=game.view==='third'?'第三人称':'第一人称';$('#view-status').textContent=`${label} · R 切换`;if(announce)toast(`已切换为${label}`)}
function toggleViewMode(){applyViewMode(game.view==='third'?'first':'third',true)}
...
function createBot(team,x,z,name){
  const started=performance.now(),{root,model,parts}=buildOperator(team,{castShadow:false});
...
document.addEventListener('keydown',e=>{
  ...
  if(game.phase==='workbench'){if(e.code==='Escape')closeWorkbench();return}
  ...
  if(e.code==='KeyR'&&!e.repeat&&game.phase==='playing'){e.preventDefault();toggleViewMode()}
...
$('#pause-guide').onclick=()=>toast(game.map?.range?'R 切换视角 · Q 开镜 · E 工作台 · 鼠标右键换弹':'WASD 移动 · R 切换视角 · Q 开镜 · F/G/H 姿态');
...
// ── 移动、碰撞与姿态 ──────────────────────────────────────────────
// ── 立体碰撞：支持踩上掩体、货箱与高架平台 ────────────────────────
const STEP_UP=.6,PLAYER_RADIUS=.4;
...
function recoverPlayerFromGeometry(){
  ...
  if(Number.isFinite(p.x)&&Number.isFinite(p.z)&&!blocked(p.x,p.z,feet,PLAYER_RADIUS,bodyH)){if(game.onGround)game.lastSafe={x:p.x,z:p.z,feet};return false}
  const limitX=(game.map?.width||game.map?.size||30)-1,limitZ=(game.map?.size||30)-1;let safe=null;for(let radius=.3;radius<=4&&!safe;radius+=.3)for(let i=0;i<24;i++){const angle=i*Math.PI/12,x=p.x+Math.cos(angle)*radius,z=p.z+Math.sin(angle)*radius;if(Math.abs(x)>limitX||Math.abs(z)>limitZ)continue;const g=groundAt(x,z,feet,PLAYER_RADIUS),nextFeet=Math.max(feet,g);if(g<=feet+STEP_UP+.05&&!blocked(x,z,nextFeet,PLAYER_RADIUS,bodyH)){safe={x,z,feet:nextFeet};break}}
...
function movePlayerWithCollision(p,sx,sz,feet,height){
  const steps=Math.max(1,Math.ceil(Math.hypot(sx,sz)/.1)),dx=sx/steps,dz=sz/steps;let moved=false;
  for(let i=0;i<steps;i++){if(!blocked(p.x+dx,p.z,feet,PLAYER_RADIUS,height)){p.x+=dx;moved=true}if(!blocked(p.x,p.z+dz,feet,PLAYER_RADIUS,height)){p.z+=dz;moved=true}}
  return moved
}
function updatePlayer(dt){
  if(!game.player.alive){if(game.playerModel)game.playerModel.root.visible=false;return}let f=(game.keys.KeyW?1:0)-(game.keys.KeyS?1:0),r=(game.keys.KeyD?1:0)-(game.keys.KeyA?1:0);const hasInput=!!(f||r),len=Math.hypot(f,r)||1;f/=len;r/=len;
  ...
  const sx=(-Math.sin(game.yaw)*f+Math.cos(game.yaw)*r)*speed*dt,sz=(-Math.cos(game.yaw)*f-Math.sin(game.yaw)*r)*speed*dt;if(game.view==='third'&&hasInput)game.moveYaw=Math.atan2(-sx,-sz);
  ...
  let feet=game.feet;
  movePlayerWithCollision(p,sx,sz,feet,bodyH);
  ...
  const moving=Math.hypot(p.x-oldX,p.z-oldZ)>.0001,moveGait=game.stance==='prone'?'prone':game.stance==='crouch'?'crouch':game.keys.ShiftLeft&&game.keys.KeyW&&!game.aiming?'run':'walk';game.playerMoving=moving;
  if(game.playerModel){const avatar=game.playerModel.root,now=performance.now()/1000,w=currentWeapon(),reload=w&&game.reloading?Math.sin(clamp(1-(game.reloadAt-now)/w.reload,0,1)*Math.PI):0;avatar.visible=game.view==='third';avatar.position.set(p.x,feet,p.z);avatar.rotation.y=game.view==='third'&&moving?game.moveYaw:game.yaw;poseOperator(avatar,{stance:game.stance,moving,gait:moveGait,reload,time:now,dt});poseHeldGun(game.playerModel,reload,dt)}
  ...
  if(game.onGround&&!blocked(p.x,p.z,feet,PLAYER_RADIUS,bodyH))game.lastSafe={x:p.x,z:p.z,feet};gunBob+=dt*(moving?speed*1.8:1.5);gunKick=Math.max(0,gunKick-dt*7);
...
function setAiming(enabled,instant=false){const active=!!enabled&&!!currentWeapon()&&game.player?.alive!==false&&!game.reloading,overlay=$('#scope-overlay');game.aiming=active;if(viewGun)viewGun.visible=game.view==='first'&&!active&&game.selected>=1&&game.selected<=2;overlay.dataset.sight=equippedSight()||'iron';overlay.classList.toggle('active',active);$('#crosshair').style.opacity=active?0:1;if(instant){camera.fov=active?aimFov():72;camera.updateProjectionMatrix()}}
...
function respawnBot(bot){bot.alive=true;bot.death=null;bot.hp=100;bot.armor=bot.rangeTarget?0:50;bot.takedownStartedAt=null;bot.target=null;bot.lastStep=performance.now()/1000;bot.spawnProtection=bot.rangeTarget?0:2;const weapon=WEAPONS[bot.weaponKey];if(weapon){bot.mag=weapon.mag;bot.reloading=false;bot.reloadTimer=0;bot.cooldown=0}bot.navPath=null;bot.navIndex=0;bot.navRepath=0;bot.navGoal=null;bot.stuckTime=0;bot.escapeTimer=0;if(bot.teamMarker)bot.teamMarker.visible=true;bot.root.rotation.set(0,0,0);bot.root.scale.set(1,1,1);bot.root.position.set(bot.rangeTarget?bot.rangeX:bot.spawn.x,0,bot.rangeTarget?bot.rangeZ:bot.spawn.z);poseOperator(bot.root,{stance:'stand',moving:false,time:performance.now()/1000,dt:1});bot.lastSafe=bot.root.position.clone();bot.parts.forEach(p=>{p.userData._solid=true});setBotAnimation(bot,'idle')}
...
function updateBots(dt){
  const animT=performance.now()/1000,idleT=animT*1.8;
  for(const b of game.entities){
    ...
    if(b.rangeTarget){b.root.position.x=b.rangeX;b.root.position.z=b.rangeZ;if(!b.alive){updateBotDeath(b,dt);if((b.respawn-=dt)<=0)respawnBot(b)}else{b.root.rotation.x=0;b.root.rotation.z=0;poseOperator(b.root,{stance:'stand',moving:false,time:animT,dt});setBotAnimation(b,'idle')}continue}
    if(b.remote){if(!b.alive){updateBotDeath(b,dt);continue}b.root.rotation.x=0;b.root.rotation.z=0;if(b.netTarget){const moving=b.netMoving!==false&&b.root.position.distanceToSquared(b.netTarget)>.0025,smooth=1-Math.exp(-dt*16),reload=b.netReloading?Math.abs(Math.sin(animT*3.4)):.0;setBotAnimation(b,moving?'walk':'idle');b.root.position.lerp(b.netTarget,smooth);const modelYaw=(b.netYaw??b.root.rotation.y)+Math.PI,turn=Math.atan2(Math.sin(modelYaw-b.root.rotation.y),Math.cos(modelYaw-b.root.rotation.y));b.root.rotation.y+=turn*smooth;poseOperator(b.root,{stance:b.netStance||'stand',moving,gait:b.netGait||'walk',reload,time:animT,dt});poseHeldGun(b,reload,dt);updateEntityFootstep(b,moving,b.netGait||b.netStance||'walk')}continue}
    ...
    const movedDistance=Math.hypot(b.root.position.x-oldX,b.root.position.z-oldZ),moving=movedDistance>.0001;if(wantedSpeed>.8&&(!moved||movedDistance<dt*.18))b.stuckTime+=dt;else b.stuckTime=Math.max(0,b.stuckTime-dt*2.4);if(b.stuckTime>.48){b.avoidSide*=-1;b.escapeTimer=.75+Math.random()*.45;b.navPath=null;b.navRepath=0;b.stuckTime=0;game.aiStats.unstucks++}const reloadWeapon=WEAPONS[b.weaponKey],reload=b.reloading&&reloadWeapon?Math.sin(clamp(1-b.reloadTimer/reloadWeapon.reload,0,1)*Math.PI):0;poseOperator(b.root,{stance:'stand',moving,gait:'walk',reload,time:animT,dt});poseHeldGun(b,reload,dt);setBotAnimation(b,moving?'walk':'idle');updateEntityFootstep(b,moving,'walk');
...
window.__RIFT_POSE_DEBUG__=()=>{const avatar=game.playerModel,rig=avatar?.root.userData.operatorRig||[],legs=rig.filter(item=>item.kind==='leg'),delta=legs.reduce((value,item)=>Math.max(value,Math.abs(item.mesh.rotation.x-item.rotation.x)),0),handDistances=[];if(avatar?.gun){avatar.root.updateWorldMatrix(true,true);const bounds=new THREE.Box3().setFromObject(avatar.gun);for(const side of[-1,1]){let nearest=Infinity;for(const item of rig)if(item.kind==='arm'&&item.section==='hand'&&item.side===side)nearest=Math.min(nearest,bounds.distanceToPoint(item.mesh.getWorldPosition(new THREE.Vector3())));handDistances.push(nearest)}}return{stance:game.stance,view:game.view,reloading:game.reloading,cameraYaw:game.yaw,facingYaw:avatar?.root.rotation.y||0,modelPitch:avatar?.model.rotation.x||0,modelY:avatar?.model.position.y||0,legRotationDelta:delta,gunInHands:!!avatar?.gun&&avatar.gun.parent===avatar.model,handDistances,gunPosition:avatar?.gun?avatar.gun.position.toArray():null,scaleY:avatar?.model.scale.y||0}};
